...
import com.crm.crmapp.order.model.RecentOrderResult

public class RecentOrderAdapter(
    var recentOrderList: ArrayList<RecentOrderResult>,
    var c: Context?,
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        viewHolder.dataBind(this!!.c!!, recentOrderList[position], position)
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun dataBind(
            context: Context,
            recentOrderList: RecentOrderResult,
            position: Int
        ) {
            ...
            tv_name.setText(recentOrderList.getCustName())
            tvDistibutor_name.setText(recentOrderList.getMainCustName())

            //tvDate.setText(recentOrderList.getOrderDate())
            tvDate.setText(ConstantVariable.parseDateTo_ddMMMyyyy_From_yyyyMMDD_WithoutTime(recentOrderList.getOrderDate()!!))

            if (recentOrderList.getStatusText().equals("New", ignoreCase = true)) {
                yvTime.setBackgroundResource(R.drawable.bg_pill_info)
            } else if (recentOrderList.getStatusText().equals("CANCELLED", ignoreCase = true)) {
                if (context != null) {
            ...
            yvTime.setText(recentOrderList.getStatusText())
            erpId.setText("ERP Ref : " + recentOrderList.getErpRefId())
            llRow.setOnClickListener {
                if (onItemClickValue != null) {
                    ConstantVariable.animationEffect(llRow, context as Activity)
                    onItemClickValue?.getPositionOfList(recentOrderList.getOrderId()!!)
                }
            }
            ...
            if (recentOrderList.getNoOfBoxes() != null) {
                tvTotalBox.setText("Total Box : " + recentOrderList.getNoOfBoxes())
